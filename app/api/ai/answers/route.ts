...
export async function POST(req: Request) {
  ...
  try {
    ...
    const { text } = await generateText({
      model: google("gemini-1.5-flash"), // stable v1 model
...
Provide the final answer in markdown format.`,
      system:
        "You are a helpful assistant that provides informative responses in markdown format. Use appropriate markdown syntax for headings, lists, code blocks, and emphasis where necessary. For code blocks, use short-form lowercase language identifiers (e.g., 'js', 'py', 'ts', 'html', 'css').",
    });
